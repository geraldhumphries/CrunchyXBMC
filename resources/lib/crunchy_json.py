...
class CrunchyJSON:
    ...
    def loadShelf(self):
        try:
            ...
            userData['API_HEADERS'] = [('User-Agent', "Mozilla/5.0 (PLAYSTATION 3; 4.46)"),
                                       ('Host', "api.crunchyroll.com"), ('Accept-Encoding', "gzip, deflate"),
        ...
        if (not userData.has_key('session_id')) or (not userData.has_key('auth_expires')) or current_datetime > \
                userData['auth_expires']:
            # Start new session
            xbmc.log("Crunchyroll;xbmc ----> Starting new session")
            opener = urllib2.build_opener()
            opener.addheaders = userData['API_HEADERS']
            options = urllib.urlencode({'device_id': userData["device_id"], 'device_type': userData['API_DEVICE_TYPE'],
            ...
            if request['error'] is False:
                ...
                xbmc.log(
                    "Crunchyroll.bundle ----> Error starting new session. Error message is: " + str(request['message']))
                return False
            ...
            if not userData['username'] or not userData['password']:
                ...
                opener = urllib2.build_opener()
                opener.addheaders = userData['API_HEADERS']
                options = urllib.urlencode({'session_id': userData['session_id'], 'password': userData['password'],
                                            'account': userData['username'], 'version': userData['API_VERSION'],
                ...
                if request['error'] is False:
                    ...
                    userData['premium_type'] = 'free' if request['data']['user']['premium'] == '' else \
                        request['data']['user']['premium']
                    xbmc.log("Crunchyroll.bundle ----> Login successful.")
                elif request['error'] is True:
                    xbmc.log("Crunchyroll.bundle ----> Error logging in new session. Error message was: " + str(
                        request['message']))
                    self.userData = userData
                    userData.close()
                    return False
        ...
        elif userData.has_key("session_id") and userData.has_key("auth_expires") and current_datetime < userData[
            'auth_expires'] and current_datetime > userData['session_expires']:
            ...
            try:
                if request['error'] is False:
                    ...
                    userData['premium_type'] = 'free' if request['data']['user']['premium'] == '' else \
                        request['data']['user']['premium']
                    userData['auth_token'] = request['data']['auth']
                    userData['session_expires'] = (current_datetime + dateutil.relativedelta.relativedelta(
                        hours=+4))  # 4 hours is a guess. Might be +/- 4.
                    userData['test_session'] = current_datetime
                    xbmc.log("Crunchyroll.bundle ----> Session restart successful. New session_id is: " + str(
                        userData['session_id']))
                    # Call for Usage Reporting
                    if current_datetime > userData['lastreported']:
                        userData['lastreported'] = (current_datetime + dateutil.relativedelta.relativedelta(hours=+24))
                    ...
                    if userData['premium_type'] in 'anime|drama|manga':
                        xbmc.log(
                            "Crunchyroll.bundle ----> User is a premium " + str(userData['premium_type']) + " member.")
                        self.userData = userData
                        userData.close()
                        return True
                    else:
                    ...
                    xbmc.log("Crunchyroll.bundle ----> Error restarting session. Error message was: " + str(
                        request['message']))
                    self.userData = userData
                    userData.Save()
                    return False
                ...
                userData['session_expires'] = current_datetime - dateutil.relativedelta.relativedelta(hours=+24)
                xbmc.log(
                    "Crunchyroll.bundle ----> Error restarting session. Error message was: " + str(request['message']))
                self.userData = userData
                userData.Save()
                return False
        ...
        # If we got to this point that means a session exists and it's still valid, we don't need to do anything.
            ...
            if userData['test_session'] is None or current_datetime > userData['test_session']:
                userData['test_session'] = (
                    current_datetime + dateutil.relativedelta.relativedelta(minutes=+10))  # test once every 10 min
                ...
                if request['error'] is False:
                    xbmc.log(
                        "Crunchyroll.bundle ----> A valid session was detected. Using existing session_id of: " + str(
                            userData['session_id']))
                    # Call for Usage Reporting
                    if current_datetime > userData['lastreported']:
                        userData['lastreported'] = (current_datetime + dateutil.relativedelta.relativedelta(hours=+24))
    ...
    def list_series(self, title, media_type, filterx, offset):
        ...
        options = {'media_type': media_type.lower(), 'filter': filterx, 'fields': fields, 'limit': '64',
                   'offset': int(offset)}
        ...
        if request['error'] is False:
            ...
            for series in request['data']:
                ...
                description = '' if series['description'] is None else series['description'].encode(
                    'utf-8')  # Adding sinopses
                thumb = '' if (series['portrait_image'] is None or series['portrait_image'][
                    'large_url'] is None or 'portrait_image' not in series or 'large_url' not in series[
                                   'portrait_image']) else series['portrait_image'][
                    'full_url']  # Becuase not all series have a thumbnail.
                art = '' if (series['landscape_image'] is None or series['landscape_image'][
                    'full_url'] is None or 'landscape_image' not in series or 'full_url' not in series[
                                 'landscape_image']) else series['landscape_image'][
                    'full_url']  # Becuase not all series have art.
                rating = '0' if (series['rating'] == '' or 'rating' not in series) else series[
                    'rating']  # Because Crunchyroll seems to like passing series without ratings
                if ('media_count' in series and 'series_id' in series and 'name' in series and series[
                    'media_count'] > 0):  # Because Crunchyroll seems to like passing series without these things
                    crunchy_main.UI().addItem(
                        {'Title': series['name'].encode("utf8"), 'mode': 'list_coll', 'series_id': series['series_id'],
                         'count': str(series['media_count']), 'Thumb': thumb, 'Fanart_Image': art, 'plot': description,
                         'year': year}, True)
            if counter >= 64:
                offset = (int(offset) + counter)
                crunchy_main.UI().addItem(
                    {'Title': '...load more', 'mode': 'list_series', 'showtype': media_type, 'filterx': filterx,
                     'offset': str(offset)})
            ...
            if filterx == 'genre':
                if 'genre' in request['data']:
                    for genre in request['data']['genre']:
                        crunchy_main.UI().addItem(
                            {'Title': genre['label'].encode("utf8"), 'mode': 'list_series', 'showtype': media_type,
                             'filterx': 'tag:' + genre['tag']}, True)

            if filterx == 'season':
                if 'season' in request['data']:
                    for season in request['data']['season']:
                        crunchy_main.UI().addItem(
                            {'Title': season['label'].encode("utf8"), 'mode': 'list_series', 'showtype': media_type,
                             'filterx': 'tag:' + season['tag']}, True)
        crunchy_main.UI().endofdirectory('none')
        ...
        if request['error'] is False:
            if len(request['data']) <= 1:
                ...
                for collection in request['data']:
                    complete = '1' if collection['complete'] else '0'
                    crunchy_main.UI().addItem(
                        {'Title': collection['name'].encode("utf8"), 'filterx': series_name, 'mode': 'list_media',
                         'count': str(count), 'id': collection['collection_id'],
                         'plot': collection['description'].encode("utf8"), 'complete': complete,
                         'season': str(collection['season']), 'series_id': series_id, 'Thumb': thumb,
                         'Fanart_Image': fanart}, True)
        crunchy_main.UI().endofdirectory('none')
    ...
    def list_media_items(self, request, series_name, season, mode, fanart):
        for media in request:
            ...
            fanart = media['series']['landscape_image']['fwide_url'] if (
                mode == "history" or mode == "queue") else fanart  # On history/queue, the fanart is get directly from the json.
            media = media[
                'media'] if mode == "history" else media  # History media is one level deeper in the json string than normal media items.
            if mode == "queue" and 'most_likely_media' not in media:  # Some queue items don't have most_likely_media so we have to skip them.
                continue
            media = media[
                'most_likely_media'] if mode == "queue" else media  # Queue media is one level deeper in the json string than normal media items.
            ...
            available_in = str(available_delta.days) + " days." if available_delta.days > 0 else str(
                available_delta.seconds / 60 / 60) + " hours."

            # Fix Crunchyroll inconsistencies & add details for upcoming or unreleased episodes
            media['episode_number'] = '0' if media['episode_number'] == '' else media[
                'episode_number']  # PV episodes have no episode number so we set it to 0.
            media['episode_number'] = re.sub('\D', '', media[
                'episode_number'])  # Because CR puts letters into some rare episode numbers.
            if media['episode_number'] == '0':
                name = "NO NAME" if media['name'] == '' else media['name']
            else:
                name = "Episode " + str(media['episode_number']) if media['name'] == '' else "Episode " + media[
                    'episode_number'] + " - " + media[
                                                                                                 'name']  # CR doesn't seem to include episode names for all media so we have to make one up.
            name = series_name + " " + name if (mode == "history" or mode == "queue") else name
            name = "* " + name if media['free_available'] is False else name
            soon = "Coming Soon - " + series_name + " Episode " + str(
                media['episode_number']) if mode == "queue" else "Coming Soon - Episode " + str(media['episode_number'])
            ...
            thumb = "http://static.ak.crunchyroll.com/i/no_image_beta_full.jpg" if media[
                                                                                       'screenshot_image'] is None else \
                media['screenshot_image']['fwide_url'] if media['free_available'] is True else \
                    media['screenshot_image'][
                        'fwidestar_url']  # because not all shows have thumbnails.
            thumb = "http://static.ak.crunchyroll.com/i/coming_soon_beta_fwide.jpg" if media[
                                                                                           'available'] is False else thumb  # Sets the thumbnail to coming soon if the episode isn't available yet.
            description = '' if media['description'] is None else media['description'].encode(
                'utf-8')  # Adding sinopses
            ...
            year = 'None' if media['available_time'] is None else media['available_time'][
                                                                  :10]  # Adding published date instead
            url = media['url']
            media_id = url.split('-')[-1]
            crunchy_main.UI().addItem({'Title': name.encode("utf8"), 'mode': 'videoplay', 'id': media_id.encode("utf8"),
                                       'Thumb': thumb.encode("utf8"), 'url': url.encode("utf8"), 'Fanart_Image': fanart,
                                       'plot': description, 'year': year, 'playhead': playhead, 'duration': duration},
                                      False)
        crunchy_main.UI().endofdirectory('none')
                ...
                for series in request['data']:
                    ...
                    if ('media_count' in series and 'series_id' in series and 'name' in series and series[
                        ...
                        crunchy_main.UI().addItem({'Title': series['name'].encode("utf8"), 'mode': 'list_coll',
                                                   'series_id': series['series_id'], 'Thumb': thumb,
                                                   'Fanart_Image': art, 'plot': description, 'year': year}, True)
    ...
    def startPlayback(self, Title, url, media_id, playhead, duration, Thumb):
        ...
        values = {'session_id': self.userData['session_id'], 'version': self.userData['API_VERSION'],
                  'locale': self.userData['API_LOCALE'], 'media_id': media_id, 'fields': fields}
        opener = urllib2.build_opener()
        ...
        if request['error'] is False:
            if request['data']['stream_data'] is not None:
                ...
                try:
                    while player.isPlaying:
                        ...
                        if x == 30:
                            ...
                            values = {'session_id': session_id, 'event': 'playback_status',
                ...
                strTimePlayed = str(int(round(timeplayed)))
                values = {'session_id': session_id, 'event': 'playback_status', 'locale': self.userData['API_LOCALE'],
    ...
    def changeLocale(self):
        ...
        if (self.userData['username'] != '' and self.userData['password'] != ''):
            ...
            data = urllib.urlencode({'formname': 'RpcApiUser_Login', 'next_url': '', 'fail_url': '/login',
                                     'name': self.userData['username'], 'password': self.userData['password']})
        ...
        url = 'https://www.crunchyroll.com/?a=formhandler'
        data = urllib.urlencode({'next_url': '', 'language': self.userData['API_LOCALE'],
    ...
    def usage_reporting(self):
        ...
        data = urllib.urlencode(
            {'entry_1580743010': self.userData['device_id'], 'entry_623948459': self.userData['premium_type'],
             'entry_1130326797': __version__, 'entry.590894822': __XBMCBUILD__})
        opener = urllib2.build_opener()
