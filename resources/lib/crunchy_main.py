...
class UI:
    ...
    def channels(self):
        ...
        self.addItem(
            {'Title': popular, 'mode': 'list_series', 'showtype': showtype, 'filterx': 'popular', 'offset': '0'})
        self.addItem(
            {'Title': Simulcasts, 'mode': 'list_series', 'showtype': showtype, 'filterx': 'simulcast', 'offset': '0'})
        self.addItem(
            {'Title': Recently_Added, 'mode': 'list_series', 'showtype': showtype, 'filterx': 'updated', 'offset': '0'})
        self.addItem({'Title': alpha, 'mode': 'list_series', 'showtype': showtype, 'filterx': 'alpha', 'offset': '0'})
        self.addItem({'Title': Browse_by_Genre, 'mode': 'list_categories', 'showtype': showtype, 'filterx': 'genre',
                      'offset': '0'})
        self.addItem(
            {'Title': seasons, 'mode': 'list_categories', 'showtype': showtype, 'filterx': 'season', 'offset': '0'})
        self.endofdirectory()

    def json_list_series(self):
        crunchy_json.CrunchyJSON().list_series(self.main.args.name, self.main.args.showtype, self.main.args.filterx,
    ...
    def json_list_collection(self):
        crunchy_json.CrunchyJSON().list_collections(self.main.args.series_id, self.main.args.name, self.main.args.count,
                                                    self.main.args.icon, self.main.args.fanart)

    def json_list_media(self):
        crunchy_json.CrunchyJSON().list_media(self.main.args.id, self.main.args.filterx, self.main.args.count,
                                              self.main.args.complete, self.main.args.season, self.main.args.fanart)
    ...
    def startVideo(self):
        crunchy_json.CrunchyJSON().startPlayback(self.main.args.name, self.main.args.url, self.main.args.id,
                                                 self.main.args.playhead, self.main.args.duration, self.main.args.icon)
